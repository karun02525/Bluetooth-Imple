package com.example.sensorble.data.ble
...
import com.example.sensorble.data.TempHumidityResult
import com.example.sensorble.data.TemperatureAndHumidityReceiveManager
...
class TemperatureAndHumidityReceiveManagerImpl @Inject constructor(
    private val bluetoothAdapter: BluetoothAdapter,
    private val context: Context

) : TemperatureAndHumidityReceiveManager {

    private val DEVICE_NAME = "karunpriya"
    ...
    private val scanCallback = object : ScanCallback() {
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            Log.d("TAGS", "onScanResult: address ${result.device.address}")
            Log.d("TAGS", "onScanResult: name" + result.device.name)

            if (result.device.name == DEVICE_NAME) {
                coroutineScope.launch {
                    data.emit(Resource.Loading(message = "Connecting to devices..."))
                }
                if (isScanning) {
                    /*
                    <-- If you have a sensor/device which
                          has ble AND normal bluetooth then call:
                          result.device.connectGatt(context,false,gattCallback, BluetoothDevice.TRANSPORT_LE)
                          Otherwise it could be possible that you cant discover the services later
                     */
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    ...
                    this@TemperatureAndHumidityReceiveManagerImpl.gatt = gatt
